...
public class ParabolaController : MonoBehaviour
{
    ...
    void Update()
    {
        if (transform.tag=="turningTrap")
        {
            transform.Rotate(rotationSpeed * Time.deltaTime,0 , 0);
        }
        else if (transform.tag == "Trap")
        {
        ...
        {
            //transform.gameObject.GetComponent<MeshRenderer>().material.DOFloat(0f, "_Alpha", 0.5f);
            //transform.gameObject.GetComponent<MeshRenderer>().material.DOFloat(0f, "_Alpha", 0.5f);
...

        {
            ...
            //transform.gameObject.GetComponent<MeshRenderer>().material.DOFloat(1f, "_Alpha", 0.5f);
            //FollowParabola();
            FollowParabola();
            // transform.gameObject.GetComponent<Material>().SetFloat("_Alpha",0);
            //Animation = true;
    ...
    public void InverseFollowParabola()
    {
        InvRefreshTransforms(Speed);
        animationTime = 0f;

        transform.position = parabolaFly.Points[parabolaFly.Points.Length].position;
    ...
    public void InvRefreshTransforms(float speed)
    {
        parabolaFly.InvRefreshTransforms(speed);
    }
    ...
    public class ParabolaFly
    {
        ...
        public void InvRefreshTransforms(float speed)
        {
            ...
            if (Points != null)
            {
                ...
                //create parabolas
                for (int i = parabolas.Length; i <=0; i--)
                {
                    if (parabolas[i] == null)
                        parabolas[i] = new Parabola3D();
